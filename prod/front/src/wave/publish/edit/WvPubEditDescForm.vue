...
const funn = {
    ...
    trashImg: (v: Form.UploadImage) => futuring(aii, async () => {
        const src: Form.UploadImage[] = prp.form.gallery || []
        const i = arrfindi(src, v.path, 'path');
        if (i >= 0) { 
            const d: ONE = src[i].data
            if (d && d.documentId) {
                const res: Media = await server_pubplus.delete_media(d as Media)
            }
            src.splice(i, 1);
        }
