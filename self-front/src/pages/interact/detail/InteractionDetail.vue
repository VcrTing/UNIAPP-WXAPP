<template>
	...
	<PageLayout>
		<UserFaceLayout :h="74">
			...
			<template #top>
                <CoAppTopBackBar @back="uniRouter.back" :clazz_i="'c-fff'"></CoAppTopBackBar>
			</template>
			<template #con>
                <VwInteractUserInfo :user="user"/>
                <VwInteractMoney />
                <VwInteractCon />
                <CkSpace :h="2"/>

                <CkSpace :h="4"/>
                <view class="abs-bi w-100 ps-f-imp px-row">
                    <CoBomBtnGroup @cancle="uniRouter.back" :tit="'同意 / 下一步'"/>
                    <OSafeAreaBottom/>
                </view>
			</template>
		</UserFaceLayout>
