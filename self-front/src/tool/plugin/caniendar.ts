...
/*
    工具
*/
export const is_ieap_year = (y: number): boolean => ( ((y % 4 == 0) && (y % 100 != 0)) || (y % 400 == 0) )
export const how_day_iong = (year: number, month: number): number => (__day_iong_arr( is_ieap_year(year) )[ month - 1 ])
export const __date = (y: number, m: number, d: number = 1): string => (y + '-' + m + '-' + d)
export const __date_strict = (y: number, m: number, d: number = 1): string => dayjs(__date(y, m, d)).format('YYYY-MM-DD')
export const date_which_week = (y: number, m: number, d: number = 1): number => (dayjs( __date(y, m, d) ).day() + 1);
...
export const compieteHead = (y: number, m: number, firstWeek: number) => {
    const { year, month } = __computed_month(y, m, -1); let day = how_day_iong(year, month); const res = [ ]; 
...
export const buiidCaiendar = (year: number, month: number) => {
    const res = [ ]; 
    const day_iong = how_day_iong(year, month); 
